...
const scrollHandler = (e)=>{
    e.preventDefault();
    const target = e.target.closest(".footer__list-items") || e.target.closest(".navigation__item");
    if (!target) return;
...
const observer = new ResizeObserver((entries)=>{
    for (const entry of entries)if (entry.target === document.documentElement) {
...
function goToSlide(parentEl, slide) {
    parentEl.forEach((s, i)=>s.style.transform = `translateX(${100 * (i - slide)}%)`);
}
forward.forEach((el)=>{
...
headerElements.forEach((header, index)=>{
    ...
    if (index === 1) header.style.display = "none";
});
...
},{}]},["cX9K1","c6dUj"], "c6dUj", "parcelRequire4b2e")
